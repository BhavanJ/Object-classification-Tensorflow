...
def main():
    ...
    pascal_classifier = tf.estimator.Estimator(
        ...
        model_dir="./tmp/pascal_vgg16_finetune_04_updated_1",
        config = tf.estimator.RunConfig(model_dir = "./tmp/pascal_vgg16_finetune_04_updated_1", 
    ...
    writer = tf.summary.FileWriter("./tmp/pascal_vgg16_finetune_04_updated_1")
#########################################################################################################################################################UPDATED####################
    pascal_classifier.train(input_fn=train_input_fn,steps=NUM_ITERS/10,hooks=[hook_restore])       #logging_hook])	########ADDED summary_hook


    for ii in range(9):
        pascal_classifier.train(input_fn=train_input_fn,steps=NUM_ITERS/10)       #logging_hook])	
